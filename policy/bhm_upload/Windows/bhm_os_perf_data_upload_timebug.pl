	...
	else
	{
		...
		while(my $line=<CODA>)
		{
			...
			if($line=~/^(\S+)\s+(\d+):(\d+):(\d+)(.*?)\|(\w+)\s*\|(.+?)\|/)
			{
					...
					my $AMPM= $5;my $metric=$6;my $value=$7;
...
sub  caculate
{
	...
	else
	{
		...
		my %hash_temp;
		my $a = 0;
		unless(open(CPU,"/usr/bin/vmstat 1 5|"))
		{
         ...
         my $i = 0;
			...
			if(@temp_name)
			{
				if($line=~/\d+\s+\d+\s+\d+/)
				{
					...
					$temp_value[$hash_temp{'id'}->{'index'}] += $temp[$hash_temp{'id'}->{'index'}];
		...
		my $cpu_idle_avg = $temp_value[$hash_temp{'id'}->{'index'}] / 4;
		$cpu_idle_avg = sprintf("%0.2f",$cpu_idle_avg);

		#caculate 
		$hash_ref->{'GBL_CPU_TOTAL_UTIL'}->{'value'} = 100 - $cpu_idle_avg;
