...
replace(replace(replace(CONVERT(VARCHAR(23),getdate(),120),'-',''),':',''),' ','') as timestamp,
 'MSSQL_GLOBAL' as class,
'MSSQL_BUFFER_CACHE_HIT_RATIO' as metric,
'MssqlInstance '+ @@servicename  as instance,
...
WHERE perf1.cntr_type not in (1073939459,1073939712) and perf1.counter_name in ('Buffer cache hit ratio')-- Don't display the divisors.