'''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
'' FILENAME:     bhm_os_win_config_removabledisksize.vbs
'' VERSION:      1.0
'' DESCRIPTION:  The script can be used to check  removable disk size and compare with  last time.
''               
'' USAGE:        cscript bhm_os_win_config_removabledisksize.vbs
''               
'' LANGUAGE:     VBScript
''
...
Function  getCurr()
	...
	Dim  strComputer,removabledisksize
	strComputer = "."
	...
	For Each objItem in colItems
		If  objItem.DriveType = 2 and objItem.size <> "" Then
Rem			wscript.echo "disktype:" & objItem.DriveType & " " &objItem.size
			
			removabledisksize = removabledisksize + objItem.size
		End if
	Next

getCurr = removabledisksize
End Function
...
FilePath = OvDataDirStr & "\bhm\bhm_OsConfig_removabledisksize.txt"  
...
If rezult <> 0 Then
	wscript.echo  "removable disk size change"
	
	msg_text = "removable disk size  had been change ,last:" & lastconfig & " current:" &currconfig
	msg_app = "bhm_os_win_config_removabledisksize.vbs"
	msg_grp = "BHM:WINOS:CONFIG"
	msg_obj = "removabledisk"
	msg_severity = "major"
