...
#usage: osspi_perl.sh  bhm_os_aix_config_cpumodel.pl 
...
sub  SendMsg
{
	...
	my $app='bhm_os_aix_config_cpumodel.pl';
	my $obj="cpu";
...
sub  getCurr
{
	...
	if(open($fl,"/usr/sbin/prtconf|"))
	{
		while($line = <$fl>)
		{
			if($line=~/Processor\s*Type:\s*(\S+)\s*/)
			{
				$value = $1;
			}
		}
...
our $bhm_os_aix_config_file = ${bhm_os_aix_config_dir}."/"."bhm_OsConfig_cpumodel.txt";
