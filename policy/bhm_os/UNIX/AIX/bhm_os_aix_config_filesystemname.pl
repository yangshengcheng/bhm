...
#usage: osspi_perl.sh  bhm_os_aix_config_filesystemname.pl 
...
sub  SendMsg
{
	...
	my $app='bhm_os_aix_config_filesystemname.pl';
	my $obj="filesystem";
...
sub  getCurr
{
	...
	{
		$SIG{ALRM} = sub{
			system("ps -ef |grep  /usr/bin/df|grep -v grep |awk '{print \$2}'|xargs kill -9");
		};
		...
		open($fl,"/usr/bin/df|");
			...
			while($line = <$fl>)
			{
				if($line=~/(\S+)\s+(\d+)\s+(\d+)\s+(\d+)\%\s+(\d+)\s+(\d+%)\s+(\S+)/)
				{
					my $name = $1;
					push(@names,$name);
				}
...
our $bhm_os_aix_config_file = ${bhm_os_aix_config_dir}."/"."bhm_OsConfig_filesystemname.txt";
