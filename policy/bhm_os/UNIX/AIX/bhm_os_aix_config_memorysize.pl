...
#usage: osspi_perl.sh  bhm_os_aix_config_memorysize.pl 
...
sub  SendMsg
{
	...
	my $app='bhm_os_aix_config_memorysize.pl';
	my $obj="memory";
...
sub  getCurr
{
	...
	if(open($fl,"/usr/sbin/prtconf|"))
	{
		while($line = <$fl>)
		{
			if($line=~/Memory\s*Size:\s*(\d+)\s*MB/i)
			{
				$value = $value + $1;
			}
		}
...
our $bhm_os_aix_config_file = ${bhm_os_aix_config_dir}."/"."bhm_OsConfig_memorysize.txt";
...
unless($flag)
{	
		&SendMsg("warning","memory size  had been changed last:${Last} MB; \ncurrent:${Curr} MB");
