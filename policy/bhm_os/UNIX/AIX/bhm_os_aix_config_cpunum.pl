...
#usage: osspi_perl.sh  bhm_os_aix_config_cpunum.pl 
...
sub  SendMsg
{
	...
	my $app='bhm_os_aix_config_cpunum.pl';
	my $obj="cpu";
...
sub  getCurr
{
	...
	if(open($fl,"/usr/sbin/prtconf|"))
	{
		while($line = <$fl>)
		{
			if($line=~/Number\s*Of\s*Processors:\s*(\d+)\s*/)
			{
				$value = $1;
			}
		}
...
our $bhm_os_aix_config_file = ${bhm_os_aix_config_dir}."/"."bhm_OsConfig_cpunum.txt";
