...
#usage: osspi_perl.sh  bhm_os_hpux_config_ipaddr.pl 
...
sub  SendMsg
{
	...
	my $app='bhm_os_hpux_config_ipaddr.pl';
	my $obj="ipaddress";
...
sub  getCurr
{
	...
	if(open($fl,"/usr/bin/netstat -in|"))
	{
		while($line = <$fl>)
		{
			if($line=~/(\S+)\s+\d+\s+\d+\.\d+\.\d+\.\d+\s+(\d+\.\d+\.\d+\.\d+)\s+\d+\s+\d+\s+\d+\s+\d+\s+\d+\s*/i)
			{
				$name = $1;$ip = $2;
				push(@names,$name."->".$ip);
			}
	...
	{
		&SendMsg("normal","invoke netstat commad fail,because $!");
		return @names ;
	}
}
...
our $bhm_os_hpux_config_file = ${bhm_os_hpux_config_dir}."/"."bhm_OsConfig_ipaddr.txt";
