...
#usage: osspi_perl.sh  bhm_os_linux_event_filesystemstat.pl -f filesystemMountPoint  -t thresholdValue -s avaliableSize
...
sub  SendMsg
{
	...
	my $app='bhm_os_linux_event_filesystemstat.pl';
	my $obj="filesystem";
...
sub  checkFileSystem
{
	...
	$SIG{ALRM} = sub{system("ps -ef |grep  /bin/df|grep -v grep |awk '{print \$2}'|xargs kill -9");};
	...
	if(open($fl,"/bin/df -m|"))
	{
		while($line = <$fl>)
		{
			if($line=~/^\S+\s+\d+\s+\d+\s+(\d+)\s+(\d+)%\s+(\S+)\s*/ || $line=~/^\s+\d+\s+\d+\s+(\d+)\s+(\d+)%\s+(\S+)\s*/)
			{
				$s_temp = $1;$t_temp = $2;$f_temp = $3;
				if($f =~/all/i)
				{
