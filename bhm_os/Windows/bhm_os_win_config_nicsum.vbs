'''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
'' FILENAME:     bhm_os_win_config_nicsum.vbs
'' VERSION:      1.0
'' DESCRIPTION:  The script can be used to check  num of nic and compare with  last time.
''               
'' USAGE:        cscript bhm_os_win_config_nicsum.vbs
''               
'' LANGUAGE:     VBScript
''
...
Function  getCurr()
	Rem query   nic  total instance
	Dim  strComputer,frequency
	strComputer = "."
	Set objWMIService = GetObject("winmgmts:\\" & strComputer & "\root\cimv2")
	
  Set nicinfo = objWMIService.ExecQuery("Select * from Win32_NetworkAdapter",,48)
		nicsum = 0
    For Each objItem in nicinfo
    If objItem.AdapterTypeID <> "" and objitem.Manufacturer <> "Microsoft" Then
     	nicsum = nicsum + 1
    End If
		Next
...
FilePath = OvDataDirStr & "\bhm\bhm_OsConfig_nicsum.txt"  
...
If rezult <> 0 Then
	wscript.echo  "network interface card hardware change"
	
	msg_text = "network interface card hardware had been changed ,last:" & lastconfig & " current:" &currconfig
	msg_app = "bhm_os_win_config_nicsum.vbs"
