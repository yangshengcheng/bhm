...
Function  getcurr()
	...
	Set IPConfigSet = objWMIService.ExecQuery("Select * from Win32_NetworkAdapter",,48)
	
	For Each IPConfig in IPConfigSet
    If Not IsNull(IPConfig.MACAddress) and IPConfig.Manufacturer <> "Microsoft" Then 
Rem     For i=LBound(IPConfig.IPAddress) to UBound(IPConfig.IPAddress)
       currIpaddr(j) = IPConfig.MACAddress
Rem     Next
			j = j + 1
    End If
...
FilePath = OvDataDirStr & "\bhm\bhm_OsConfig_macaddr.txt" 
...
If rezult <> 0 Then
	wscript.echo  "mac address change"
	...
	msg_text = "mac address had been change," & " " & last_text & " " & curr_text
	msg_app = "bhm_os_win_config_macaddr.vbs"
