...
#usage: osspi_perl.sh  bhm_os_aix_config_logicdisksize.pl 
...
sub  SendMsg
{
	...
	my $app='bhm_os_aix_config_logicdisksize.pl';
	my $obj="physicdisk";
...
sub  getCurr
{
	...
	if(open($fl,"/usr/sbin/prtconf|"))
	{
		while($line = <$fl>)
		{
			if($line=~/\+\s+hdisk\d+\s+\S+\s+.*?\(\s*(\d+)\s*MB\)/)
			{
				$value = $value + $1;
			}
		}
...
our $bhm_os_aix_config_file = ${bhm_os_aix_config_dir}."/"."bhm_OsConfig_logicdisksize.txt";
...
unless($flag)
{	
		&SendMsg("warning","local  physic disk size  had been changed last:${Last} MB; \ncurrent:${Curr} MB");
