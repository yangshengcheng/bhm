...
#usage: osspi_perl.sh  bhm_os_aix_config_macaddr.pl 
...
sub  SendMsg
{
	...
	my $app='bhm_os_aix_config_macaddr.pl';
	my $obj="macaddress";
...
sub  getCurr
{
	...
	if(open($fl,"/usr/bin/netstat -in|"))
	{
		while($line = <$fl>)
		{
			if($line=~/(\S+)\s+\d+\s+\S+\s+(\w+\.\w+\.\w+\.\w+\.\w+\.\w+)\s+/)
			{
				my $name = $1."->".$2;
				push(@names,$name);
			}
...
our $bhm_os_aix_config_file = ${bhm_os_aix_config_dir}."/"."bhm_OsConfig_macaddr.txt";
