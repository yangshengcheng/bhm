...
#usage: osspi_perl.sh  bhm_os_aix_config_manufacturer.pl 

#parameter: none

#description: check manufacturer config 
...
sub  SendMsg
{
	...
	my $app='bhm_os_aix_config_manufacturer.pl';
	my $obj="manufacturer";
...
sub  getCurr
{
	...
	if(open($fl,"/usr/sbin/prtconf|"))
	{
		while($line = <$fl>)
		{
			if($line=~/System\s*Model:\s*(\S+)\s*/i)
			{
				$value = $1;
			}
		}
	...
	{
		&SendMsg("normal","invoke prtconf commad fail");
		return 0;
	}
}
...
our $bhm_os_aix_config_file = ${bhm_os_aix_config_dir}."/"."bhm_OsConfig_manufacturer.txt";
...
unless($flag)
{	
		&SendMsg("warning","manufacturer had been changed last:${Last}; \ncurrent:${Curr}");
