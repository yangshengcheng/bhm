...
#usage: osspi_perl.sh  bhm_os_aix_config_cpufrequency.pl 
...
sub  SendMsg
{
	...
	my $app='bhm_os_aix_config_cpufrequency.pl';
	my $obj="cpu";
...
sub  getCurr
{
	...
	if(open($fl,"/usr/sbin/prtconf|"))
	{
		while($line = <$fl>)
		{
			if($line=~/Processor\s*Clock\s*Speed:\s*(\S+)\s*/i)
			{
				$value = $1;
			}
		}
...
our $bhm_os_aix_config_file = ${bhm_os_aix_config_dir}."/"."bhm_OsConfig_cpufrequency.txt";
...
unless($flag)
{	
		&SendMsg("warning","cpu frequency had been changed last:${Last} MHZ; \ncurrent:${Curr} MHZ");
