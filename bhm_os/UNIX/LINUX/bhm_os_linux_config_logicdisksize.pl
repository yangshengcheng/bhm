...
#usage: osspi_perl.sh  bhm_os_linux_config_logicdisksize.pl 
...
sub  SendMsg
{
	...
	my $app='bhm_os_linux_config_logicdisksize.pl';
	my $obj="physicdisk";
...
sub  getCurr
{
	...
	if(open($fl,"/sbin/fdisk|"))
	{
		while($line = <$fl>)
		{
			if($line=~/Disk\s+\S+\s*:\s*\S+\s*GB\s*,\s*(\d+)\s*bytes/)
			{
				$value = $value + $1;
			}
		}
...
our $bhm_os_linux_config_file = ${bhm_os_linux_config_dir}."/"."bhm_OsConfig_logicdisksize.txt";
...
unless($flag)
{	
		&SendMsg("warning","local  physic disk size  had been changed last:${Last} bytes; \ncurrent:${Curr} bytes");
