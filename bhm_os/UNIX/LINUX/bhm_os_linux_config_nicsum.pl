...
#usage: osspi_perl.sh  bhm_os_linux_config_nicsum.pl 
...
sub  SendMsg
{
	...
	my $app='bhm_os_linux_config_nicsum.pl';
	my $obj="nic";
...
sub  getCurr
{
	...
	my %names=();
	my @names=();
	if(open($fl,"/sbin/ifconfig -a|"))
	{
		while($line = <$fl>)
		{
			if($line=~/^(\w+)\s+Link\s+encap\s*:/)
			{
				my $name = $1;
				$names{$name}=1;
			}
...
our $bhm_os_linux_config_file = ${bhm_os_linux_config_dir}."/"."bhm_OsConfig_nicsum.txt";
